...
describe('single()', () => {
    it('should returns the only element of a sequence', () => {
        ...
        assert.strictEqual(new Enumerable([1][Symbol.iterator]()).single(), 1);
    ...
    it('should throws an exception if there is not exactly one element in the sequence', () => {
        ...
        assert.throws(() => new Enumerable([][Symbol.iterator]()).single());
        assert.throws(() => new Enumerable([1, 2]).single());
        assert.throws(() => new Enumerable([1, 2][Symbol.iterator]()).single());
